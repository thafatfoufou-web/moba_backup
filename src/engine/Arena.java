...
public class Arena {
    ...
    public Arena() {
        ...
        Enemy_Fountain = new Fountain(0.95, 0.05, new Color(202, 94, 90));
   ...
   public void update(double deltaTime) {
    ...
    for (Bot ebot : enemyBots) {
        ebot.update();
        List<Entity> targets = getEnemiesForTeam(ebot.getTeam());
        Entity closest = null;
        double closestDist = Double.MAX_VALUE;
        for (Entity e : targets) {
            double d = ebot.getDistanceTo(e);
            if (d < closestDist) { closestDist = d; closest = e; }
        }
    ...
    for (Minion m : minionSpawner.getMinions()) {
        List<Entity> enemies = getEnemiesForTeam(m.getTeam());
